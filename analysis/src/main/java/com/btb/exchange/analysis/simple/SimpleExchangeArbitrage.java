...
public class SimpleExchangeArbitrage {
    ...
    private static <T> Predicate<T> distinctByKeys(Function<? super T, ?>... keyExtractors) {
        ...
        return t -> {
            final List<?> keys =
                    Arrays.stream(keyExtractors).map(ke -> ke.apply(t)).toList();
    ...
    Opportunities process(ExchangeOrderBook orderBook) {
        Optional<BigDecimal> askPrice =
                orderBook.getOrders().getAsks().stream().findFirst().map(Order::getLimitPrice);
        Optional<BigDecimal> bidPrice =
                orderBook.getOrders().getBids().stream().findFirst().map(Order::getLimitPrice);
     ...
     * Find opportunities
     *
     * <p>TODO amount of opportunity should contain amount of incoming order.
     */
    private Opportunities findOpportunities(ExchangeOrderBook orderBook) {
        ...
        final BigDecimal ask = orderBook.getOrders().getAsks().stream()
                .findFirst()
                .map(Order::getLimitPrice)
                .orElse(BigDecimal.ZERO);
        final BigDecimal bid = orderBook.getOrders().getBids().stream()
                .findFirst()
                .map(Order::getLimitPrice)
                .orElse(BigDecimal.ZERO);
        ...
        asks.entrySet().stream()
                ...
                .forEach(e -> opportunitiesBuilder.value(new Opportunity(
                        currencyPair,
                        e.getKey().getExchange(),
                        e.getValue(),
