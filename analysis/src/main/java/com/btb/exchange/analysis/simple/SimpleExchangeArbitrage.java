...
public class SimpleExchangeArbitrage {
  ...
  public SimpleExchangeArbitrage(
      ExchangeService exchangeService, HazelcastInstance hazelcastInstance) {
  ...
  Opportunities merge(List<Opportunities> opportunitiesList) {
    ...
    var distinct =
        opportunities.stream()
            .filter(
                distinctByKeys(
                    Opportunity::getCurrencyPair, Opportunity::getFrom, Opportunity::getTo))
            .toList();
  ...
  private Opportunities findOpportunities(ExchangeOrderBook orderBook) {
    ...
    final BigDecimal ask =
        orderBook.getOrders().getAsks().stream()
            .findFirst()
            .map(Order::getLimitPrice)
            .orElse(BigDecimal.ZERO);
    final BigDecimal bid =
        orderBook.getOrders().getBids().stream()
            .findFirst()
            .map(Order::getLimitPrice)
            .orElse(BigDecimal.ZERO);
