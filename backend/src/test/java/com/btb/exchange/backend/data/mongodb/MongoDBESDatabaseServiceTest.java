...
class MongoDBESDatabaseServiceTest {
  ...
  @Autowired MongoDBDatabaseService service;
  @Autowired MongodbMessageRepository repository;
  ...
  void testReplayMessage() throws InterruptedException, JsonProcessingException {
    ...
    assertThat("result before timeout", waitResult);
    // because in this test case the replayed records will be stored again, so at least 2 is the
    // answer
    assertThat(
        "check #records are added",
        repository.count().blockingGet() - startCount,
