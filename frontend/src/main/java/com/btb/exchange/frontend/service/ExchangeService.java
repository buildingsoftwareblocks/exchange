...
public class ExchangeService {
    ...
    ExchangeService(
        ...
        opportunityDelay =
                DistributionSummary.builder("frontend.opportunity.delay").register(registry);
    }
    ...
    @KafkaListener(topics = TopicUtils.INPUT_TICKER, containerFactory = "batchFactory", groupId = GROUP_ID)
    ...
    @KafkaListener(topics = TopicUtils.OPPORTUNITIES, containerFactory = "batchFactory", groupId = GROUP_ID)
    ...
    public Optional<String> tickersData() {
        if (tickers.isEmpty()) {
            ...
            Comparator<ExchangeTicker> comparator =
                    Comparator.comparing(ExchangeTicker::getCurrencyPair).thenComparing(ExchangeTicker::getExchange);
