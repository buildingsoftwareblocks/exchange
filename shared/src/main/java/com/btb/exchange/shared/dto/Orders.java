...
public class Orders {
    ...
    public Orders(List<LimitOrder> asks, List<LimitOrder> bids, int maxOrders) {
        ...
        sublist(asks, maxOrders)
                .forEach(ask -> this.asks.add(new Order(ask.getLimitPrice(), ask.getOriginalAmount())));
        sublist(bids, maxOrders)
                .forEach(bid -> this.bids.add(new Order(bid.getLimitPrice(), bid.getOriginalAmount())));
    }
